...
void solve() { 
    ...
    vector<int> carNumber(numberOfCars + 1);
    vector<int> relativePosition(numberOfCars + 1);
    vector<int> originalCarPosition(numberOfCars + 1, -1);
    ...
    for(int i = 1; i <= numberOfCars; i++) {
        ...
        cin >> carNumber[i];
        cin >> relativePosition[i];
    }

    for(int i = 1; i <= numberOfCars && ans; i++) {

        int positionGainedOrLoss = relativePosition[i];

        if( (i + positionGainedOrLoss) > numberOfCars  ||
         (i + positionGainedOrLoss) <= 0 || 
         originalCarPosition[i + positionGainedOrLoss] != -1)
                ans = false;
            
        else
            originalCarPosition[i + positionGainedOrLoss] = carNumber[i];
    }
