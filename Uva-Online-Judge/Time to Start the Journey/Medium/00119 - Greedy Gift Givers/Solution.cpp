...
void solve() { 
    ...
    int n, amountPersonSpent, numberOfGiftsGiven;
    cin >> n;
    unordered_map<string, int> mp;
    string nameOfAllPersons[n];
    string temp, nameOfPerson;

    for(int i = 0;  i < n; i++) {

        cin >> temp;
        mp[temp] = 0;
        nameOfAllPersons[i] = temp;
    }
    ...
    for(int i = 0; i < n; i++) {
        ...
        cin >> nameOfPerson;
        cin >> temp;
        amountPersonSpent = stoi(temp);
        cin >>temp;
        numberOfGiftsGiven = stoi(temp);

        int spentPerPerson = amountPersonSpent / numberOfGiftsGiven;
        mp[nameOfPerson] -= amountPersonSpent;


        for(int j = 0; j < numberOfGiftsGiven; j++) {

            cin >> temp;
            mp[temp] += spentPerPerson;
        }
    }

    for(int i = 0; i < n; i++)
        cout<<nameOfAllPersons[i]<<" "<<mp[nameOfAllPersons[i]]<<endl;
