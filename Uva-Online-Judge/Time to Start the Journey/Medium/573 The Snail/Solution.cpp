...
void solve() { 
    ...
    while(true){
        ...
        int day = 1; 
        double snailCurrDistance = u - d;
        

        while(snailCurrDistance < h && snailCurrDistance >= 0) {

            day++;

            double percent = (100 - (day - 1) * f) / 100.0;
            if(percent < 0) {
            ...
            double snailIncrement = u * percent;
            snailCurrDistance += snailIncrement;
            if(snailCurrDistance > h)
                break;
