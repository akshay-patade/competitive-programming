...
struct Proposal{
    ...
    bool operator<(const Proposal& other) const {
        ...
        if(price == other.price && compliant == other.compliant)
            return true;
        
        else if(compliant == other.compliant)
            return price < other.price;
        
        return compliant > other.compliant;
    }
};
...
void solve() { 
    ...
    int numberOfRequirements, numberOfProposals;
    cin >> numberOfRequirements >> numberOfProposals;
    cin.ignore();

    string requirements[numberOfRequirements];
    string temp;
    ...
    for(int i = 0; i < numberOfProposals; i++) {
        ...
        p.price = stod(temp.substr(0, pos));
        p.compliant = stoi(temp.substr(pos + 1, temp.length() - pos - 1));

       
        for(int j = 0; j < p.compliant; j++)
            getline(cin, temp);
