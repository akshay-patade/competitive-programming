...
void solve() { 
    ...
    int numberOfDevices, numberOfOperations, maximumCapacity, deviceNumber;
    cin >> numberOfDevices >> numberOfOperations >> maximumCapacity;

    int capacityOfEachDevice[numberOfDevices + 1];

    for(int i = 1; i <= numberOfDevices; i++)
        cin >>capacityOfEachDevice[i];

    int currCapacity = 0;
    int maximumCapacityReached = 0;

    set<int> isOn;

    bool fuzeBlown = false;

    for(int i = 0; i < numberOfOperations; i++) {

        cin >> deviceNumber;

        //Device was on and you are turning it off
        if(isOn.find(deviceNumber) != isOn.end()) {

            isOn.erase(deviceNumber);
            currCapacity -= capacityOfEachDevice[deviceNumber];
        }

        //fuze blown
        else if(currCapacity + capacityOfEachDevice[deviceNumber] > maximumCapacity) {
        ...
        //It can handle the capacity
            ...
            currCapacity += capacityOfEachDevice[deviceNumber];
            isOn.insert(deviceNumber);
            maximumCapacityReached = max(maximumCapacityReached, currCapacity);
        }
    }
    ...
    if(fuzeBlown)
        ...
        cout<<"Maximal power consumption was " << maximumCapacityReached <<" amperes."<<endl;
    }
