...
void solve() { 
    ...
    int availableBeds[hotels][weeks];
    ...
    for(int i = 0; i < hotels; i++) {
        ...
        for(int j = 0; j < weeks; j++) {
            ...
            cin >> availableBeds[i][j];
            if(participants <= availableBeds[i][j] && cost > 1LL * participants * hotelPrice[i])
                cost = min(cost, 1LL * participants * hotelPrice[i]);
        }
    }
