...
void solve() { 
    ...
    int loanDuration, numDepRecords, x;
    double downPayment, y, loanAmount, monthlyPayment;

    cin >> loanDuration >> downPayment>> loanAmount >> numDepRecords; 
    pair<int, double> depRecords[numDepRecords];

    for(int i = 0;  i < numDepRecords; i++) {

        cin >> x >> y;
        depRecords[i] = make_pair(x, y);
    }

    sort(depRecords, depRecords + numDepRecords);

    monthlyPayment = loanAmount / loanDuration;

    double carValue = loanAmount +  downPayment;
    carValue *= (1 - depRecords[0].second);

    int i = 0;
    int currentDepCounter = 0;

    while(carValue < loanAmount) {

        i++;
        if(currentDepCounter < numDepRecords && i == depRecords[currentDepCounter + 1].first)
            currentDepCounter++;
        
        loanAmount -= monthlyPayment;
        carValue *= (1 - depRecords[currentDepCounter].second);
    }
